...
describe('evidenceApi', () => {
  it('normalizes the base URL and sends the canonical presign payload', async () => {
  ...
  it('uses PUT for file content and POST for canonical evidence metadata', async () => {
    ...
    const file = new File(['evidence'], 'record.pdf', { type: 'application/pdf' })
  ...
  it('parses list and detail responses through their stable routes', async () => {
    ...
    expect(fetchMock.mock.calls[0][0]).toBe('https://api.example.com/evidence')
    expect(fetchMock.mock.calls[1][0]).toBe(
      'https://api.example.com/evidence/record%2F1',
    )
  })
