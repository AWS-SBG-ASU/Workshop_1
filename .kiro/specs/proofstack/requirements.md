...
- Workshop IAM permissions shall grant only these actions: presign `s3:PutObject`; create `dynamodb:PutItem`; list `dynamodb:Query` and, after signed downloads are enabled, `s3:GetObject`; get `dynamodb:GetItem` and `s3:GetObject`; delete `dynamodb:GetItem`, then `s3:DeleteObject`, then `dynamodb:DeleteItem`. Each workshop policy shall use `"Resource": "*"`. Production access must target the exact table ARN and the exact `evidence/demo/*` object ARN.
