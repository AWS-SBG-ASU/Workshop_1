...
- Use API Gateway HTTP API payload format 2.0. Parameterized routes read `pathParameters.id`.
...
- Return JSON with `Content-Type: application/json` and the configured CORS origin; a successful delete returns an empty `204`.
...
- Write the failing local test first. After every handler change, run local tests and a Lambda Console test using a payload format 2.0 event.
