...
Complete every AWS action in the AWS Management Console in `us-east-1`. Use `<account-id>` for the 12-digit AWS account ID and choose globally unique bucket names such as `proofstack-assets-<account-id>-us-east-1` and `proofstack-web-<account-id>-us-east-1`. Record resource names, ARNs, origins, and invoke URLs outside version control.

Complete the five phases in order.

## 1. Create the HTTP API shell

1. Open **API Gateway** → **APIs** → **Create API**.
2. Under **HTTP API**, choose **Build**.
3. Set **API name** to `ProofStackApi`. Skip integrations and routes.
4. Create the `$default` stage with **Auto-deploy** enabled, then create the API.
5. Open **CORS** → **Configure**. Set allowed origin to `http://localhost:5173`, allowed methods to `GET`, `POST`, `DELETE`, and `OPTIONS`, allowed headers to `content-type` and `accept`, and max age to 300 seconds.
6. Record the default endpoint: `https://<api-id>.execute-api.us-east-1.amazonaws.com`.

## 2. Create five Python Lambdas and connect the routes
...
Create one Lambda integration per function with payload format version **2.0**, create the routes exactly as shown, and attach each integration. Item events use `pathParameters.id`. Confirm `$default` auto-deploys and the API appears under each Lambda's triggers.
...
1. Set `VITE_API_BASE_URL` in an uncommitted production environment file to the API endpoint without a trailing slash.
...
7. In API Gateway CORS, add the exact website origin alongside `http://localhost:5173`; preserve the phase-1 methods, headers, and max age.
8. In the private asset bucket CORS rule, add the exact website origin alongside localhost; preserve `GET`, `HEAD`, and `PUT`. Keep Block Public Access enabled.
9. Update `ALLOWED_ORIGIN` on all five Lambdas to the exact website origin used by deployed responses.
10. Open the website and exercise presign/PUT, create, newest-first list, get/download, and delete. Public access remains limited to the frontend bucket.